...
import { api } from '../services/api'; // Utilisation du client API centralisé
...
const getEmbedUrl = (url) => {
  ...
  if (match && match[1]) {
    // Correction de la syntaxe du template literal pour l'URL d'intégration YouTube
...
const ResourceForm = ({ resourceName, onSuccess, onCancel, formFields }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      ['categories', 'tags', 'category'].forEach(field => {
        if (dataToSend[field] && typeof dataToSend[field] === 'string') {
          dataToSend[field] = dataToSend[field].split(',').map(s => s.trim());
        }
      });
      
      // Utilisation de la fonction 'api' pour gérer l'envoi de données
      const newResource = await api(`/api/${resourceName}`, {
        method: 'POST',
        body: dataToSend,
      });
  ...
  return (
    <div className="p-6 max-w-xl mx-auto">
      ...
      {message && (
        <div className={`p-3 mb-4 rounded-lg text-center ${message.includes('réussi') ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
          {message}
        </div>
      )}
...
const ResourceList = ({ resourceName, renderCard, renderModalContent, formFields, placeholder, canDelete = true }) => {
  ...
  useEffect(() => {
    const fetchResources = async () => {
      try {
        // Utilisation de la fonction 'api' pour récupérer les ressources
        const data = await api(`/api/${resourceName}`);
        setResources(data);
      } catch (err) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      // Utilisation de la fonction 'api' pour la suppression
      await api(`/api/${resourceName}/${resourceToDelete.id}`, {
        method: 'DELETE',
      });
      ...
      console.error('Erreur:', err);
      // alert(`Erreur: ${err.message}`); // Remplacer par une modale
...
function RessourcesPage() {
  ...
  return (
    <div className="min-h-screen bg-cover bg-center bg-no-repeat bg-fixed" style={{ backgroundImage: "url('https://res.cloudinary.com/djhyztec8/image/upload/v1755533696/rendu-3d-d-une-conception-d-arriere-plan-pour-les-communications-reseau_l2k5vc.jpg')", backgroundColor: "rgba(255, 255, 255, 0.8)", backgroundBlendMode: "lighten", }}>
          ...
          {ressources.map(r => {
            ...
            return (
              <Link
                key={r.path}
                to={r.path}
                className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${isActive ? 'bg-brand-brown text-white' : 'bg-transparent border border-neutral-300 text-neutral-700 hover:bg-neutral-50'}`}
              >
                {r.label}
              </Link>
            );
