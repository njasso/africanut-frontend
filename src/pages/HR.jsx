...
const API_URL = 'https://africanut-backend-production.up.railway.app';
...
export default function HR() {
  ...
  const sortedEmployees = [...filteredEmployees].sort((a, b) => {
    const fieldA = sortField === 'company' ? a.company?.name || '' : a[sortField] || '';
    const fieldB = sortField === 'company' ? b.company?.name || '' : b[sortField] || '';
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-between items-center mt-4">
            <div className="flex items-center gap-2">
              <span>Afficher :</span>
              <select value={itemsPerPage} onChange={e => { setItemsPerPage(Number(e.target.value)); setCurrentPage(1); }} className="px-2 py-1 border rounded">
            ...
            <div className="flex justify-center items-center gap-2">
              <button disabled={currentPage===1} onClick={()=>setCurrentPage(p=>p-1)} className="px-3 py-1 rounded bg-gray-200 hover:bg-gray-300">&lt;</button>
              {Array.from({length: totalPages}, (_, i) => (
                <button key={i} onClick={()=>setCurrentPage(i+1)} className={`px-3 py-1 rounded ${currentPage===i+1?'bg-gray-900 text-white':'bg-gray-200 hover:bg-gray-300'}`}>{i+1}</button>
              ))}
              <button disabled={currentPage===totalPages} onClick={()=>setCurrentPage(p=>p+1)} className="px-3 py-1 rounded bg-gray-200 hover:bg-gray-300">&gt;</button>
            </div>
          </div>
        </div>
      </div>
