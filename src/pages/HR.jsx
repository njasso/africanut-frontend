...
const fetchWithAuth = async (url, options = {}) => {
  ...
  if (!response.ok) {
    ...
    throw new Error(errText || `Erreur API ${response.status}`);
  }
...
export default function HR() {
  ...
  const loadEmployees = useCallback(async ()=>{
    ...
    try {
      const data = await fetchWithAuth(`${API_BASE}/api/employees`);
      setItems(data);
    } catch(err) {
  ...
  const addOrUpdate = async e=>{
    ...
    try{
      ...
      if(editMode && currentEmployeeId){
        await fetchWithAuth(`${API_BASE}/api/employees/${currentEmployeeId}`, {
          method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload)
        });
      } else {
        await fetchWithAuth(`${API_BASE}/api/employees`, {
          method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload)
        });
      }
      loadEmployees(); resetForm(); setEditMode(false); setCurrentEmployeeId(null);
  ...
  const confirmDelete = async ()=>{
    ...
    setIsLoading(true); setError(null);
    try{
      await fetchWithAuth(`${API_BASE}/api/employees/${employeeToDelete.id}`, { method:'DELETE' });
      loadEmployees(); setShowDeleteModal(false); setEmployeeToDelete(null);
    } catch(err){ setError(err.message); } finally{ setIsLoading(false); }
  };
  const cancelDelete = ()=>{ setShowDeleteModal(false); setEmployeeToDelete(null); };

  const startEdit = employee=>{
    setEditMode(true); setCurrentEmployeeId(employee.id);
    setForm({
      name:employee.name||'',
      role:employee.role||'',
      ...
      <div className="grid md:grid-cols-[340px,1fr] gap-6">
        ...
        <form onSubmit={addOrUpdate} className="bg-white rounded-2xl p-5 shadow-lg space-y-3">
          ...
          <div className="flex justify-end gap-2 pt-4">
            {editMode && <button type="button" className="px-4 py-2 bg-gray-300 rounded-xl" onClick={()=>{setEditMode(false); setCurrentEmployeeId(null); resetForm();}}>Annuler</button>}
            <button type="submit" className="px-4 py-2 bg-gray-900 text-white rounded-xl">{isLoading?"Chargement...":(editMode?"Mettre à jour":"Ajouter")}</button>
          </div>
        </form>
